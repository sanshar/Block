 ...
 const SpinBlock* b, SparseMatrix* o, opTypes state_index_ij)
{
  ...
  for (int opind=0; opind<opvec1.size(); opind++) {
    ...
    SpinAdapted::operatorfunctions::TensorProduct(otherblock, *op3, *op1,
                                                  b, &(b->get_stateInfo()), o[ilock], factor, numthrds);
    ...
    if (otherblock == b->get_rightBlock())
      ...
      SpinAdapted::operatorfunctions::TensorProduct(otherblock, Transposeview(*op3), Transposeview(*op1),
                                                    b, &(b->get_stateInfo()), o[ilock], factor, numthrds);
    }
  }
}
 ...
 const SpinBlock* b, SparseMatrix* o, opTypes state_index_ij)
{
  ...
  for (int opind=0; opind<opvec1.size(); opind++) {
    ...
    SpinAdapted::operatorfunctions::TensorProduct(otherblock, *op2, *op1,
                                                  b, &(b->get_stateInfo()), o[ilock], parity*factor, numthrds);
    ...
    SpinAdapted::operatorfunctions::TensorProduct(otherblock, Transposeview(*op2), Transposeview(*op1),
                                                  b, &(b->get_stateInfo()), o[ilock], parity*factor, numthrds);  
  }
}
 ...
 const SpinBlock* b, SparseMatrix* o, opTypes state_index_ij)
{
  ...
  for (int opind=0; opind<opvec1.size(); opind++) {
    ...
    SpinAdapted::operatorfunctions::TensorProduct(otherblock, *op2, Transposeview(*op1),
                                                  b, &(b->get_stateInfo()), o[ilock], scale*parity, numthrds);
    ...
    SpinAdapted::operatorfunctions::TensorProduct(otherblock, Transposeview(*op2), *op1,
                                                  b, &(b->get_stateInfo()), o[ilock], scale*parity, numthrds);
  }
}
 ...
 const SpinBlock* b, Wavefunction& c, Wavefunction* v, const SpinQuantum& q, opTypes state_index_ij)
{
  ...
  for (int opind=0; opind<opvec1.size(); opind++) {
    ...
    if (otherblock == b->get_rightBlock())
      parity = getCommuteParity(-op1->get_deltaQuantum(), -op2->get_deltaQuantum(), hq);
    SpinAdapted::operatorfunctions::TensorMultiply(otherblock, Transposeview(*op2), Transposeview(*op1),
                                                   b, c, v[ilock], hq, factor*parity);  
  }
}
 ...
 const SpinBlock* b, Wavefunction& c, Wavefunction* v, const SpinQuantum& q, opTypes state_index_ij)
{
  ...
  for (int opind=0; opind<opvec1.size(); opind++) {
    ...
    SpinAdapted::operatorfunctions::TensorMultiply(otherblock, *op2, Transposeview(*op1), b, c, v[ilock], hq, scale*parity);
    ...
    //if (otherblock == b->get_leftBlock())
    //parity = getCommuteParity(op1->get_deltaQuantum(), op2->get_deltaQuantum(), q)*-1.0;
    SpinAdapted::operatorfunctions::TensorMultiply(otherblock, Transposeview(*op2), *op1, b, c, v[ilock], hq, scale*parity);
  }
}
