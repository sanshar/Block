...
void GeneralMatrix::ReSize(int nr, int nc, Real* d)
{
  //REPORT
   if (store)
   {
   ...
   storage=nr*nc; nrows=nr; ncols=nc; tag=-1;
   if (storage)
   {
     store = d; //MatrixErrorNoSpace(store);
     //MONITOR_REAL_NEW("Make (ReDimensi)",storage,store)
   }
   else store = 0;
}
...
void Matrix::ReSize(int nr, int nc, Real* d)
{ REPORT GeneralMatrix::ReSize(nr,nc,d); }
