...
For threepdm and fourpdm, there might be too many elements. \texttt{disk\_dump\_pdm} can be used to store npdm elements on disk, rather than in memory. It is in file \texttt{spatial\_threepdm.n.n.p.bin}. p is the processor number. The data structure is a long integer ( index) and a double float (npdm element). They are not ordered and elements with zero value are not stored. The index begins with 0 for (0,0,0,0,0,0) and 1 for (0,0,0,0,0,1). By default, threepdm and fourpdm without summing up the spins are not stored, since spatial pdm is enough for many situations. \texttt{store\_spinpdm} can be set to get them. 

To calculate transition density matrix between states with different symmetry irrepresentation, several steps are needed. 
