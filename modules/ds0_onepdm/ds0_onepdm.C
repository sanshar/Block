...
pout<< " check  opQ" << opQ<<endl;

    Wavefunction opw2;
    vector<SpinQuantum> dQ = wave1.get_deltaQuantum();
pout<< " check  opQ" << dQ[0]<<endl;
    opw2.initialisebra(dQ, &big, true);
//    operatorfunctions::TensorMultiply(leftBlock, *op, &big, wave2, opw2, dQ[0], 1.0);
...
pout<< " check  opQ" << dQ[0]<<endl;
    opw2.initialisebra(dQ, &big, true);
    operatorfunctions::TensorMultiply(leftBlock, *op, *overlap, &big, wave2, opw2, opQ, 1.0);
//    operatorfunctions::TensorMultiply(leftBlock, *op, &big, wave2, opw2, opQ, 1.0);
    double sum = DotProduct(wave1, opw2);
...
//---
  ...
  {
    boost::shared_ptr<SparseMatrix> op = rightBlock->get_op_array(DES_CRE).get_local_element(ij)[0]->getworkingrepresentation(rightBlock);
    int ix = op->get_orbs(0);
    int jx = op->get_orbs(1);
