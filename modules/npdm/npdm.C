                         ...
                         const bool &restart, const int &restartSize, const int state)
{
  ...
  if(dmrginp.setStateSpecific()){
    if (!restart) SpinBlock::store (forward, system.get_sites(), system, state, state ); // if restart, just restoring an existing block --
  }
  else{
...
// Old onepdm and twopdm are not used for quantum chemistry type calculation.
// If you want to use it, just change dmrginp.new_npdm_code()
//void npdm_restart( int npdm_order )
//{
//  bool direction;
//  int restartsize;
...
void transition_pdm( int npdm_order )
{
  ...
  if( !dmrginp.setStateSpecific()){
    ...
    //    } 
    //    else{
    //      if (npdm_order == 1) transition_onepdm::do_one(sweepParams, false, direction, false, 0, state);     
    //      else if (npdm_order == 2) transition_twopdm::do_one(sweepParams, false, direction, false, 0, state);
    //      else abort();
    //    }
    //   }
    }
