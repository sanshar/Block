...
//FIXME move all parallel code into a separate file??
...
{
assert(false); // serial version not tested
  // Many spatial combinations on left block
...
#endif
    //cout << "-------------------------------------------------------------------------------------------\n";
    cout << "Doing pattern " << count << " of " << patterns.size() << endl;
...
void Npdm_driver::compute_npdm_elements(std::vector<Wavefunction> & wavefunctions, const SpinBlock & big, int sweepPos, int endPos)
{
  ...
  Timer timer;
  cout << "===========================================================================================\n";
  cout << "Current NPDM sweep position = "<< sweepPos+1 << " of " << endPos+1 << "\n";

  // Loop over NPDM operator patterns
  Npdm_patterns npdm_patterns( npdm_order_, sweepPos, endPos );
  Npdm_expectations npdm_expectations( spin_adaptation_, npdm_patterns, npdm_order_, wavefunctions.at(0), big );
  ...
  if (mpigetrank() == 0) {
    ...
    cout << "NPDM calls to expectation engine " << sum << endl;
  } else {
  ...
  if (mpigetrank() == 0) {
    ...
    cout << "NPDM mpi communications time " << sum << endl;
  } else {
    reduce(world, DEBUG_COMM_TIME[mpigetrank()], std::plus<double>(), 0);
  }
#endif
  cout << "NPDM compute elements time " << timer.elapsedwalltime() << " " << timer.elapsedcputime() << endl;
  cout << "===========================================================================================\n";
