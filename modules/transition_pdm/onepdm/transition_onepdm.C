...
//   cout << "wave1 and wave2 are different";
  for (int i = 0; i < leftBlock->get_op_array(CRE).get_size(); ++i)
  for (int j = 0; j < dotBlock->get_op_array(DES).get_size(); ++j)
  {
    boost::shared_ptr<SparseMatrix> op1 = leftBlock->get_op_array(CRE).get_local_element(i)[0]->getworkingrepresentation(leftBlock);//spin 0
    boost::shared_ptr<SparseMatrix> op2 = dotBlock->get_op_array(DES).get_local_element(j)[0]->getworkingrepresentation(dotBlock);//spin 0
    int ix = op1->get_orbs(0);
    int jx = op2->get_orbs(0);
    ...
    leftop1.allocate(big.get_leftBlock()->get_stateInfo());
    //FIXME
    // only transposeview of op2 works. op2=DES do not work. DES only give zero. If change the minus of deltaQuantum into plus of deltaQuantum, it works.
  ...
  for (int j = 0; j < dotBlock->get_op_array(CRE).get_size(); ++j)
  {
    boost::shared_ptr<SparseMatrix> op1 = leftBlock->get_op_array(DES).get_local_element(i)[0]->getworkingrepresentation(leftBlock);//spin 0
    boost::shared_ptr<SparseMatrix> op2 = dotBlock->get_op_array(CRE).get_local_element(j)[0]->getworkingrepresentation(dotBlock);//spin 0
    int ix = op1->get_orbs(0);
    int jx = op2->get_orbs(0);
...
//#ifndef DNDEBUG
//      //FIXME
//      //test the transpose of pdm
//      {
//      boost::shared_ptr<SparseMatrix> test_op1 = leftBlock->get_op_array(DES).get_local_element(i)[0]->getworkingrepresentation(leftBlock);
//      boost::shared_ptr<SparseMatrix> test_op2 = rightBlock->get_op_array(CRE).get_local_element(j)[0]->getworkingrepresentation(rightBlock);
//      int ix2 = test_op1->get_orbs(0);
//      int jx2 = test_op2->get_orbs(0);
//      vector<SpinQuantum> opQ2 = test_op1->get_deltaQuantum(0)-test_op2->get_deltaQuantum(0);
//      Wavefunction test_opw2;
//      vector<SpinQuantum> dQ2 = wave1.get_deltaQuantum();
//      test_opw2.initialise(dQ, &big, true);
//      operatorfunctions::TensorMultiply(leftBlock, *test_op1, *test_op2, &big, wave2, test_opw2, opQ2[0], 1.0);
//      double test_sum = sqrt(2.0)*DotProduct(wave1, test_opw2);
//
//      if(&wave1==&wave2){
//         cout << "true"<< ix<<','<<jx<<":"<<sum<<endl;
//         cout << "true"<< ix2<<','<<jx2<<"transpose:"<<test_sum<<endl;
//      }
//      else{
//         cout << "false"<< ix<<','<<jx<<":"<<sum<<endl;
//         cout << "false"<< ix2<<','<<jx2<<"transpose:"<<test_sum<<endl;
//      }
//      }
//#endif
