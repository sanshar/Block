            ...
            for(int m=0; m<threepdm.dim5()/2; ++m)
              for(int n=0; n<threepdm.dim6()/2; ++n) {
                ...
                for (int s=0; s<2; s++)
                  for (int t =0; t<2; t++)
                    for (int u =0; u<2; u++) {
//FIXME spin summation
                      pdm += factor * threepdm(2*i+s, 2*j+t, 2*k+u, 2*l+u, 2*m+t, 2*n+s);
//                      if ( (i==0)&& (j==1)&& (k==1)&& (l==3)&& (m==3)&& (n==2) ) 
 //                       std::cout << pdm << "\t\t" <<  threepdm(2*i+s, 2*j+t, 2*k+u, 2*l+u, 2*m+t, 2*n+s) << std::endl;
                    }
              ...
              for(int n=0; n<threepdm.dim6()/2; ++n) {
                ...
                for (int s=0; s<2; s++)
                  for (int t =0; t<2; t++)
                    for (int u =0; u<2; u++)
//FIXME spin summation
                      pdm(i, j, k, l, m, n) += factor * threepdm(2*i+s, 2*j+t, 2*k+u, 2*l+u, 2*m+t, 2*n+s) * factor;
              }
