...
for (int m=0; m<2*dmrginp.last_site(); ++m) {
for (int n=0; n<2*dmrginp.last_site(); ++n) {
  if ( abs(threepdm(i,j,k,l,m,n)) > 1e-12 ) 
    std::cout << "maw-so-3pdm  " 
              << i << "," << j << "," << k << "," << l << "," << m << "," << n 
              << "\t\t" << threepdm(i,j,k,l,m,n) << std::endl;
}   
}
}
}
