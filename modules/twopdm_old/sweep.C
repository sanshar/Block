...
void SweepTwopdm::BlockAndDecimate (SweepParams &sweepParams, SpinBlock& system, SpinBlock& newSystem, const bool &useSlater, const bool& dot_with_sys, int state)
{
  ...
  if (sweepParams.get_block_iter() == 0)
    Twopdm_old::compute_twopdm_initial(solution, system, systemDot, newSystem, newEnvironment, big, numprocs, state);

  Twopdm_old::compute_twopdm_sweep(solution, system, systemDot, newSystem, newEnvironment, big, numprocs, state);

  if (sweepParams.get_block_iter()  == sweepParams.get_n_iters() - 1)
    Twopdm_old::compute_twopdm_final(solution, system, systemDot, newSystem, newEnvironment, big, numprocs, state);
...
double SweepTwopdm::do_one(SweepParams &sweepParams, const bool &warmUp, const bool &forward, const bool &restart, const int &restartSize, int state)
{
  ...
  for (int i=0; i<nroots; i++)
    Twopdm_old::save_twopdm_binary(twopdm, i, i); 
  ...
  int i = state, j = state;
  //for (int j=0; j<=i; j++) 
  Twopdm_old::load_twopdm_binary(twopdm, i, j); 
  //calcenergy(twopdm, i);
  Twopdm_old::save_twopdm_text(twopdm, i, j);
  Twopdm_old::save_spatial_twopdm_text(twopdm, i, j);
  Twopdm_old::save_spatial_twopdm_binary(twopdm, i, j);
