...
DMRG calculation for a given $M$.  Orbital reordering is performed by Block
using the keyword \texttt{reorder}, followed by a filename containing the new
orbital ordering.  
The \texttt{reorder} key is optional. In this case, we wish to arrange the orbitals
such that they follow a snake-like structure as in Fig.~\ref{fig:order}
...
However, this can be avoided since we have implemented two ordering mechanisms. If one does not add a \texttt{reorder} keyword, then the Fiedler vector automatically
reorders the orbitals (\texttt{fiedler} keyword is equivalent).

For a more robust ordering, add instead the keyword \texttt{gaopt default}. This
performs a genetic algorithm calculation to effectively find an ideal ordering.
...
%Modify
We currently have implemented a default sweep schedule. These parameters have proved robust for large systems, although theref
